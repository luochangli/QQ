...
public class ChatActivity extends Activity implements OnClickListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		updateMsgToReaded();
		XmppUtil.setCallbackChatMsg(new IReceiveChatMsg() {

			@Override
			public void updateChatRoom(
					org.jivesoftware.smack.packet.Message message) {
				Msg msg = getChatInfoTo(message.getBody(), Const.MSG_TYPE_TEXT,
						message.getFrom());
	...
	void sendMsgImg(String imgpath) {
		...
		final String message = YOU + Const.SPLIT + I + Const.SPLIT
				+ Const.MSG_TYPE_IMG + Const.SPLIT + imgpath + Const.SPLIT
				+ sd.format(new Date());
		new Thread(new Runnable() {
			@Override
			public void run() {
				try {
					XmppUtil.sendMessage(QQApplication.xmppConnection, message,
							YOU);
	...
	void sendMsgText(String content) {
		...
		input.setText("");
		final String message = YOU + Const.SPLIT + I + Const.SPLIT
				+ Const.MSG_TYPE_TEXT + Const.SPLIT + content + Const.SPLIT
				+ sd.format(new Date());
		new Thread(new Runnable() {
			@Override
			public void run() {
